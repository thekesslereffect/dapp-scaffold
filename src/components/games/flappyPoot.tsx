  ...
  scene("start", () => {
    ...
    if (publicKey ) {
      ...
      scene("game", () => {
        let score = 0;
        setGravity(1600);
        let GROUND_SPEED_NEW = GROUND_SPEED * GAME_SCALE;
        let PIPE_SPACING_NEW = PIPE_SPACING;
        const SPEED_INCREMENT = 0.5;  // Adjust this value for speed increment
        const SPAWN_INTERVAL_INCREMENT = SPEED_INCREMENT / 100;  // Adjust this value for spawn interval increment
        let currentSpeedIncrement = 0;
        let currentSpawnIntervalIncrement = 0;
        ...
        let pipeSpawnLoop = loop(PIPE_SPACING_NEW, () => {
            // producePipes();
        });
        ...
        // Loop to increase speed and adjust pipe spawn interval
        loop(2, () => {
            currentSpeedIncrement += SPEED_INCREMENT;
            GROUND_SPEED_NEW += currentSpeedIncrement;
    
            currentSpawnIntervalIncrement += SPAWN_INTERVAL_INCREMENT;
            PIPE_SPACING_NEW += currentSpawnIntervalIncrement;
    
            pipeSpawnLoop.cancel(); // Use cancel() instead of clear()
            pipeSpawnLoop = loop(PIPE_SPACING_NEW, () => {
                producePipes();
            });
        });
    });
           ...
           <div className='flex flex-col  items-center gap-4'>
              ...
              <ul className='flex flex-col text-xl w-full gap-2'>
                  {leaderboard.map((entry, index) => (
                      <a key={index} href={'https://solscan.io/account/'+entry.walletAddress} target='_blank' rel="noreferrer" className='flex flex-row justify-between gap-4 cursor-pointer'><div>{formatAddress(entry.walletAddress)}</div><div>{entry.score}</div></a>
                  ))}
              </ul>
          </div>
        </div>
